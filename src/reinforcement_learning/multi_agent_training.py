...
def train_agent(train_params, train_env_params, eval_env_params, obs_params):
    name = f"sweep_actual_final_sarsa_dqn_{train_params.policy}" \
        f"_env_{train_params.training_env_config}_obstreedepth"\
        f"_{train_params.obstreedepth}_hs_{train_params.hidden_size}"\
        f"_nstep_{train_params.n_step}_gamma_{train_params.gamma}"
    ...
    if use_graph_observator:
        ...
        # Calculate the state size given the depth of the tree
        # observation and the number of features
    ...
    if training_params.policy == "dqn" or training_params.policy == "baseline":
    ...
    if restore_replay_buffer:
        try:
            ...
            print(
                "\n🛑 Could't load replay buffer, were the "
                // "experiences generated using the same tree depth?"
            )
            print(e)
            exit(1)
    ...
    if save_replay_buffer and (hdd.free / (2**30)) < 500.0:
        print(
            "⚠️  Careful! Saving replay buffers will quickly consume"
            // " a lot of disk space. You have {:.2f}gb left.".format(
                hdd.free / (2**30)
            )
        )
    ...
    for episode_idx in range(n_episodes):
        ...
        for step in range(max_steps):
            ...
            for agent in train_env.get_agent_handles():
                if info["action_required"][agent]:
                    update_values[agent] = (
                        True  # only learn from timesteps where somethings
                    )
                    if training_params.policy == "baseline":
                        # do nothing, L, F, R, B
                        min_dist_to_target = agent_obs[agent][31:35]
                        action = np.argmin(min_dist_to_target) + 1
                    else:
        ...
        # Print logs
        if (
            episode_idx % checkpoint_interval == 0
            and not training_params.policy == "baseline"
            and not train_params.checkpoint
        ):
            torch.save(
        ...
        # ⏱️ Rst 0.023st 0 Stp 0.076sp 0.0 Lrn 0.241sn 0.1
        # Prc 0.022sc 0.0 Tot 68.886s
        print(
            "\r🚂 Ep {}"
            "\t 🏆 Score: {:.3f}"
            " Avg: {:.3f}"
        ...
        if (
            ...
            wandb.log(
                {"evaluation/scores_min": np.min(scores), "step": episode_idx}
            )
            wandb.log(
                {"evaluation/scores_max": np.max(scores), "step": episode_idx}
            )
            wandb.log(
                {
            ...
            wandb.log(
                {"evaluation/scores_std": np.std(scores), "step": episode_idx}
            )
            wandb.log(
                {"evaluation/scores": np.array(scores), "step": episode_idx}
            )
            wandb.log(
                {
        ...
        wandb.log(
            {"training/completion": np.mean(completion), "step": episode_idx}
        )
        wandb.log(
            {
        ...
        wandb.log(
            {"training/buffer_size": len(policy.memory), "step": episode_idx}
        )
        ...
        wandb.log(
            {"timer/total": training_timer.get_current(), "step": episode_idx}
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_graph_observator",
        help="custom graph observator",
        default=False,
        type=bool,
    )
    parser.add_argument(
        "--num_gnn_layers", help="number of gnn layers", default=1, type=int
    )
    parser.add_argument(
        "--renderspeed",
    ...
    obs_params = {
        ...
        # the predictor here only returns n locations of the predicted
        # agent's b and c... position
    }
    ...
    def check_env_config(id):
        if id >= len(env_params) or id < 0:
            print(
                ...
                "only Test_0 to Test_{} are supported.".format(
                    len(env_params) - 1
                )
            )
            exit(1)
    ...
    training_params.checkpoints = \
        "checkpoints/sweep_actual_final_sarsa_dqn_" \
        "sarsa_env_0_obstreedepth_2_hs_512_nstep_1_gamma" \
        "_0.99240710160404-14200.pth"
    train_agent(
        training_params,
    ...
    # python reinforcement_learning/multi_agent_training.py --n_episodes=10
    # --hidden_size=512 --buffer_size=128 --training_env_config=15
    # --policy="sarsa" --obstreedepth=2
    # --checkpoint="checkpoints/sweep_actual_final_
    # sarsa_dqn_sarsa_env_0_obstreedepth_2_hs_512_nstep_1_gamma_
    # 0.99240710160404-14200.pth" --render=True --renderspeed=100
