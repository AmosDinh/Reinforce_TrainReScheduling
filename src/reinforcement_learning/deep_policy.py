...
from reinforcement_learning.model import (
    DuelingQNetwork,
    QNetwork,
    QGINWithPooling,
)
...
class DeepPolicy(Policy):
    ...
    def __init__(
        self, state_size, action_size, parameters, evaluation_mode=False
    ):
    ...
    def _initialize(self):
        ...
        if self.dueling_dqn or self.double_dueling_dqn:
            if self.parameters.use_graph_observator:
                raise ValueError(
                    "Dueling DQN is not supported with graph observator"
                )
            else:
            ...
            raise ValueError(
                "One of double_dueling_dqn, dueling_dqn, double_dqn, " //
                "dqn, sarsa must be True"
            )

        if self.parameters.use_graph_observator:
            self.qnetwork_local = Network_Type(
                self.state_size,
                self.action_size,
                hidden_dim=self.hidsize,
                num_layers=self.parameters.num_gnn_layers,
            ).to(self.device)
        else:
            self.qnetwork_local = Network_Type(
                self.state_size,
    ...
    def step(self, state, action, reward, next_state, next_action, done):
        ...
        self.memory.add(
            ...
            self.parameters.use_graph_observator,
        )
        ...
        if self.t_step == 0:
            ...
            if (
                len(self.memory) > self.buffer_min_size
                and len(self.memory) > self.batch_size
                and len(self.memory) > self.n_step * (self.batch_size // 10)
            ):
                self._learn()
    ...
    def _calc(self, experiences):
        ...
        assert (
            ...
            == 1
        ), "Exactly one of double_dueling_dqn, dqn, must be True"
        if self.double_dueling_dqn or self.double_dqn:
            # off-policy
            # Double DQN
            # Loss = //
            # E[(r + γ * Q_target(s', argmax_{a'}(Q_local(s',a')) //
            # - Q_local(s, a))**2]
            # 1. Get the best action for the next state from the local model
            # 2. Get the Q values for the best action
            # (selected by local model) of the target model
            q_best_action = self.qnetwork_local(S_t1).max(1)[1]
            q_targets_next = self.qnetwork_target(S_t1).gather(
            ...
            q_targets_next = (
                self.qnetwork_target(S_t1).detach().max(1)[0].unsqueeze(-1)
            )
        elif self.sarsa:
            # on policy
            # SARSA
            ...
            # as update policy (could use e greedy as well) and stay
            # with e-greedy for the decision policy
            with torch.no_grad():
                self.qnetwork_local.eval()
                numerator = torch.exp(
                    self.qnetwork_local(S_t1) / self.expected_sarsa_temperature
                )
                denominator = torch.sum(numerator, dim=1).unsqueeze(1) ** -1
                pi_a = numerator * denominator
                self.qnetwork_local.train()

            q_targets_next = torch.sum(
                torch.multiply(pi_a, self.qnetwork_local(S_t1))
            ).unsqueeze(-1)
        ...
        # Compute loss
        # Learn bellmans equation with mse: Q(s,a) = r + γ * //
        # max_{a'}(Q(s', a')) -> minimize Q(s,a) - //
        # (r + γ * max_{a'}(Q(s', a')))
        # This is equivalent to saying Q(s,a) = Q(s,a) + (⍺) //
        #  (r + γ * max_{a'}(Q(s', a')) - Q(s,a))
    ...
    def _learn(self):
        if self.parameters.use_graph_observator:
            ...
            experiences = self.memory.sample(
                n=self.n_step, gamma=self.gamma, use_graph_observator=True
            )
...
class ExpectedSARSA(DeepPolicy):
    def __init__(
        ...
        expected_sarsa_temperature=1.0,
    ):
        super().__init__(state_size, action_size, parameters, evaluation_mode)
