...
def norm_obs_clip(
    obs, clip_min=-1, clip_max=1, fixed_radius=0, normalize_to_range=False
):
    """returns
    This function returns the difference between min and max value
    of an observation.
...
def _split_subtree_into_feature_groups(
    node, current_tree_depth: int, max_tree_depth: int
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    if node == -np.inf:
        remaining_depth = max_tree_depth - current_tree_depth
        # reference: https://stackoverflow.com/questions/515214/total-number-of-nodes-in-a-tree-data-structure # noqa: E501
...
def normalize_observation(observation, tree_depth: int, observation_radius=0):
    ...
    # [6*features node 1,
    # 6*features node 1.1, n1.1.1, n.1.1.2, n1.2, n1.3, n1.4]
    # meaning features are concatenated (one dimension only), and  node 1.1.s
    # children are listed before node 1.2, 1.3, 1.4
